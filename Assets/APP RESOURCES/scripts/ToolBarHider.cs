...
public class ToolbarHider : MonoBehaviour {
    ...
    private void Start() {
        ...
        AdjustWindowSizeForResolution();
    }

    // Adjust window size for specific resolutions
    private void AdjustWindowSizeForResolution() {
        // Example for small window resolution like 450x450
        int targetWidth = 450;
        ...
        // If resolution is smaller than desired, resize the window and hide borders
        if (screenWidth <= targetWidth && screenHeight <= targetHeight) {
            SetWindowPos(hWnd, HWND_TOPMOST, 0, 0, (short)targetWidth, (short)targetHeight, SWP_NOMOVE | SWP_NOSIZE | SWP_SHOWWINDOW);
            ShowWindowBorders(false); // Hide the borders after resizing
        }
    }
}
